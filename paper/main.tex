...
confidence on the same judged items. No numbers are reported here until that
measurement is complete. The comparison is pre-declared: the same items, the same
protocol, the same metrics, one model instead of the collaborative judge.

The judge prompt is the official one used elsewhere in this paper, unchanged, with a
single addition: an instruction to report a confidence value for the comparison,
