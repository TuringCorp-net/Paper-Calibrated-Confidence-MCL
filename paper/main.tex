...
On the public judgment benchmarks the collaborative judge's raw accuracy ties with a
single-model baseline of the same generation (JudgeBench 92.5\% vs 92.7\%; CJB 67.1\%
...
against 57.4 for the direct baseline and 55.6 for the official reference draft on the
...
\item
  A single model used as a judge over 76 head-to-head comparisons picked the candidate
  in the second position at a rate 7.9 points higher than the candidate in the first.
\end{itemize}
...
the collaborative generator scores 63.3 against 57.4 for the direct baseline and 55.6
for the official reference draft. In a separate comparison the same tasks were put to
the collaborative judge as a rubric-blind preference question --- the task and two
drafts, no rubric, no scores --- and the collaborative draft was chosen in 36 of 38
tasks, at a median confidence of 85.9\%.
...
\item
  ...
  collaborative judge's raw accuracy ties with a single-model baseline. The paper's
...
\item
  ...
  endpoints; the baseline re-run for H4 will use a later version of the same endpoint
  than the original runs. This is disclosed rather than corrected.
\end{enumerate}
...
benchmarks the collaborative judge's accuracy ties with a single-model baseline while
...
\item
  JudgeBench. \emph{Judging between candidate answers across MMLU-Pro, LiveBench and
  LiveCodeBench.} 2026.
\item
  Zhang, L. \emph{MCL conceptual framework.} Preprint, 2026.
  \url{https://doi.org/10.6084/m9.figshare.30645869}
\item
  Zhang, L. \emph{MCL-Team: Minimal Multi-Model Collaboration Surpasses Single Models
  on LiveBench and IFEval.} Preprint, 2026.
